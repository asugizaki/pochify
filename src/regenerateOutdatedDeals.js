...
async function fetchOutdatedDeals() {
  const allDeals = [];
  let page = 0;
  ...
  while (true) {
    ...
    const { data, error } = await supabase
      .from("deals")
      .select("*")
      .or(`layout_version.is.null,layout_version.lt.${CURRENT_LAYOUT_VERSION}`)
    ...
    allDeals.push(...data);
    console.log(`📦 Loaded outdated batch ${page + 1}: ${data.length} deals`);
...
async function fetchAllDealsForSitemap() {
  const allDeals = [];
  let page = 0;
...
async function run() {
  ...
  console.log(`📚 Outdated deals loaded: ${deals.length}`);
  ...
  console.log(`✅ Regenerated outdated: ${successSlugs.length}`);
  if (failures.length) {
  ...
  const allDeals = await fetchAllDealsForSitemap();
  await generateSitemapFromDeals(allDeals);

  console.log("🏁 Outdated regeneration complete");
}

run().catch((error) => {
  console.error("❌ regenerateOutdatedDeals fatal error:", error);
  process.exit(1);
});
